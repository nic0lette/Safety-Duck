...
safetyduck.showFirefoxContextMenu = function (event) {
	...
	// Build the menu
	for (var channel in safetyduck.channels()) {
		...
		var channelLabel = channel.toLowerCase().replace(/^(.)|\s(.)/g, function ($1) {
		...
		// Attach the command
		child.addEventListener("command", function (cmd) {
			safetyduck.onMenuItemCommand(cmd.target.label.toLowerCase());
		}, true);
...
window.addEventListener("load", function () {
	safetyduck.onFirefoxLoad();
}, false);
window.addEventListener("unload", function () {
	safetyduck.onFirefoxUnload();
}, false);